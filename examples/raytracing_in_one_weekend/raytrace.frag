...
float random (vec2 st) {
     return fract(tan(distance(st*PHI, st))*st.x);
}
...
bool raycast(const in ray r, inout hit h){
  ...
  mat pl_mat = mat(vec3(.8,.8,.8), 1., 0);
  sphere pl = sphere(vec3(0.,-1000, 0.), 1000., pl_mat);

  mat s_mat = mat(vec3(.1,.2,.6), 1., 0);
  sphere s = sphere(vec3(-4.,1.,-2.), 1., s_mat);

  mat m_mat1 = mat(vec3(.7, .6, .5), .0, 1);
  sphere met = sphere(vec3(3.,1.,-2.), 1., m_mat1);

  mat g_mat = mat(vec3(1.), 1.5, 2);
  sphere glass = sphere(vec3(0.,1.,-2.), 1., g_mat);
  ...
  for(int i=-5; i < 5; i++){
    for(int j=-3; j<3; j++){
      float seed = float(i * j + j + i*3);
      vec2 pos = random2(seed);
      vec3 center = vec3(pos.x + float(i*2), 0.2, pos.y + float(j*2) - 2.);
      mat m;
      if(pos.x < .8){
        m = mat(random3(seed), 1., 0);
      }else if(pos.x < 0.95){
        m = mat(vec3(1.), 1.5, 2);
      }else{
        m = mat(random3(seed), .0, 1);
      }
      sphere rand_s = sphere(center, .2, m);
      is_hit = hit_sphere(rand_s, r, t_min_max, h) || is_hit;
    }
  }
