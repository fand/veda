...
bool raycast(const in ray r, inout hit h){
  ...
  mat m_mat1 = mat(vec3(.8, .6, .2), .0, 1);
  sphere met = sphere(vec3(2.,0.,-2.), 1., m_mat1);

  mat m_mat2 = mat(vec3(.8), .3, 1);
  sphere met2 = sphere(vec3(-2.,0.,-2.), 1., m_mat2);

  bool is_hit = false;
  is_hit = hit_sphere(pl, r, t_min_max, h) || is_hit;
  is_hit = hit_sphere(met, r, t_min_max, h) || is_hit;
  is_hit = hit_sphere(met2, r, t_min_max, h) || is_hit;
  is_hit = hit_sphere(s, r, t_min_max, h) || is_hit;
  return is_hit;
}
vec3 jitter(){
  vec3 jitter = random_in_unit_sphere(g_seed);
