...
export default class ThreeShader {
  ...
  _createRenderPass(fs: ?string, vs: ?string, pass: any) {
    const scene = new THREE.Scene();
    const camera = new THREE.OrthographicCamera(-1, 1, 1, -1, 0.1, 10);
    camera.position.set(0, 0, 1);
    camera.lookAt(this._scene.position);

    const plane = this._createPlane(fs, vs);
    scene.add(plane);
    ...
    if (pass.TARGET) {
      ...
      this._uniforms[pass.TARGET] = texture;
      target = {
        name: pass.TARGET,
        texture: texture,
      };
  ...
  loadShader(fs: ?string, vs: ?string, passes: any[]): void {
    // Dispose old targets
    this._passes.forEach(pass => {
      if (pass.target) {
        pass.target.texture.texture.dispose();
      }
    });

    // Create new Passes
    this._passes = passes.map(pass => this._createRenderPass(fs, vs, pass));
    if (passes.length < 1) {
      this._passes = [this._createRenderPass(fs, vs, {})];
    }
  }
  ...
  _resize = () => {
    ...
    this._targets.forEach(t => t.setSize(width / this._pixelRatio, height / this._pixelRatio));
    this._uniforms.resolution.value.x = width / this._pixelRatio;
    this._uniforms.resolution.value.y = height / this._pixelRatio;
  }
  ...
  _render(): void {
    ...
    this._passes.forEach((pass: RenderPass, i: number) => {
      ...
      if (pass.target) {
        this._uniforms[pass.target.name].value = DUMMY_TEXTURE;
        this._renderer.render(pass.scene, pass.camera, pass.target.texture, true);
        this._uniforms[pass.target.name].value = pass.target.texture.texture;
      } else {
        this._renderer.render(pass.scene, pass.camera);
      }
    });
