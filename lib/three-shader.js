...
export default class ThreeShader {
  ...
  _createRenderPass(shader: string, pass) {
  ...
  loadShader(shader: string, passes: any[]): void {
    // Dispose old targets
    ...
    // Create new Passes
    this._passes = passes.map(pass => this._createRenderPass(shader, pass));
    if (passes.length < 1) {
      this._passes = [this._createRenderPass(shader, {})];
    }
  }
