...
export default class ThreeShader {
  ...
  constructor(ratio: number, skip: number) {
    ...
    this._uniforms = {
      ...
      spectrum: { type: 't', value: this._audio.spectrum },
      samples: { type: 't', value: this._audio.samples },
      midi: { type: 't', value: this._midi.texture },
      note: { type: 't', value: this._midi.noteTexture },
    };
  ...
  _render(): void {
    ...
    if (this._audio.isPlaying) {
      this._audio.update();
      this._uniforms.volume.value = this._audio.getVolume();
    }
