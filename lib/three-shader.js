...
export default class ThreeShader {
  ...
  _createRenderPass(fs: ?string, vs: ?string, pass: any): RenderPass {
    ...
    let target;
    if (pass.TARGET) {
      this._uniforms[pass.TARGET] = DUMMY_TEXTURE;
      target = {
        name: pass.TARGET,
        targets: [
          new THREE.WebGLRenderTarget(
  ...
  _render(): void {
    ...
    const lastPass = this._passes.length - 1;
    ...
    this._passes.forEach((pass: RenderPass, i: number) => {
      ...
      if (i === lastPass) {
        this._renderer.render(pass.scene, pass.camera, this._targets[1], true);
      }
    });
  }
