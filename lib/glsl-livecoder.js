...
export default class GlslLivecoder {
  ...
  constructor(config: Config) {
    ...
    this._config = config;
    this._config.on('change', ({ added, removed }) => {
      Object.keys(removed.IMPORTED).forEach(key => {
        this._three.unloadTexture(key, removed.IMPORTED[key].PATH);
      });
      ...
      if (added.midi !== undefined) {
        this._three.toggleMidi(added.audio);
      }
  ...
  loadShaderOfEditor(editor: TextEditor): void {
    ...
    validator(this._glslangValidatorPath, shader, postfix)
      .then(() => {
        ...
        this._config.setCommentByString(headComment);
      })
      .then(() => {
