...
export default class GlslLivecoder {
  ...
  constructor(config: Config) {
    ...
    this._player = new Player(view, rc, false);
  ...
  _onChange = ({ newConfig, added, removed }: RcDiff) => {
    ...
    if (added.server !== undefined) {
      ...
      if (added.server) {
        this._player = new PlayerServer(added.server, rc, this._state.isPlaying, this._config.projectPath);
      } else {
        const view = new View(atom.workspace.element);
        this._player = new Player(view, rc, this._state.isPlaying);
      }
    }
